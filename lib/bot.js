...
var bot = module.exports = bb({
    ...
    .use('before', function (ctx) {
        ...
        if (!ctx.session.volume) {
            ctx.data.volume = "Choose volume";
        } else {
            ctx.data.volume = ctx.session.volume
        }
        if (!ctx.session.symbol) {
            ctx.data.symbolName = "Choose a symbol";
        } else {
            ctx.data.symbolName = ctx.session.symbol.symbolName
        }
        var account = ctx.session.account;
        if (!account) {
            ctx.data.accountLabel = "Choose an account";
        } else {
            ctx.data.accountLabel = (account.live ? "Live" : "Demo") + " " + account.accountNumber + " " + account.brokerTitle;
        }
    });
...
bot.command('buy')
    .invoke(function (ctx) {
        ctx.sendMessage('The /buy command is not implemented yet');
    });

bot.command('sell')
    .invoke(function (ctx) {
        ctx.sendMessage('The /sell command is not implemented yet');
    });

bot.command('chart')
    .invoke(function (ctx) {
        ctx.sendMessage('The /chart command is not implemented yet');
    });
