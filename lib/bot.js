...
var bot = module.exports = bb({
    ...
    .use('before', function (ctx) {
        ...
        if (!/^settings_/.test(ctx.command.name)) {
            ...
            } else if (!ctx.session.access_token) {
...
bot.command('orders')
    .use('beforeInvoke', function (ctx) {
        return AccountsAPI.getPendingOrders(ctx.session.account.accountId, ctx.session.access_token).then(
            function (result) {
                console.log(result.body.data);
                ctx.session.pendingOrders = result.body.data;
            }
        );
    })
    .invoke(function (ctx) {
        var keyboard = ctx.session.pendingOrders.reduce(
            function (keyboard, pendingOrder) {
                var orderLabel = pendingOrder.tradeSide + " " + pendingOrder.volume + " " + pendingOrder.symbolName;
                var button = {};
                button[orderLabel] = new String(pendingOrder.orderId);
                var closeButton = {'button.pendingOrder.close' : {value: {action: 'close', orderId: pendingOrder.orderId, volume: pendingOrder.volume}}};
        ...
        keyboard.push('backButton');
        ctx.keyboard(keyboard);
        ctx.data.pendingOrdersCount =  ctx.session.pendingOrders.length;
        ctx.sendMessage('main.pendingOrders');
    })
    .answer(function (ctx) {
        if (ctx.answer && ctx.answer.action) {
            switch (ctx.answer.action) {
                case 'close' :
                    TradingAPI.cancelOrderRequest({
                        accessToken: ctx.session.access_token,
                        accountId: ctx.session.account.accountId,
                        orderId: ctx.answer.orderId
                    });
                    break;
                default :
                    ctx.sendMessage('TODO: action ' + ctx.answer.action + ' is not supported yet');
