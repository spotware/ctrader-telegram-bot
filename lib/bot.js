...
var bot = function(app, token, webhookUrl) {
...
bot.prototype.handle = function(req, res) {
    ...
    if (command && command.startsWith("/")) {
        this.redis.get("tokens." + chatId, function (err, reply) {
            if (!token) {
                command = "/auth";
            }
            switch (Message.text) {
                ...
                case "/auth":
                    ...
                    break;
                case "/accounts":
                    this.accountsAPI.getTradingAccounts(tokens[chatId].token.access_token,
...
bot.prototype.saveToken = function(token, state) {
    this.redis.put("tokens." + state, token);
