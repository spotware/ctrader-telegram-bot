        ...
        this.redisClient.get("tokens:" + chatId, function (err, token) {
            ...
            switch (text) {
                ...
                case "/stop":
                    ...
                    break;
                case "/auth":
                case emoji.emojify(":key:Log In"):
                case emoji.emojify(":bust_in_silhouette:Sign Up"):
    ...
    this.accountsAPI.getProfile(token.token.access_token,
        function (error, result) {
            if (error) {
                ...
                self.redisClient.set("profile:" + token.token.access_token, result.body);
                self.redisClient.set("profile:" + chatId, result.body);
            }
        });
